...
function buildPdfLines(result, officers, loans, generatedAt) {
  ...
  }) || [`Generated: ${generatedAtLabel}`, `Fairness engine: ${getSelectedFairnessEngineLabel()}`];
  const lines = [
...
function writePdfLines(doc, lines, result = null, options = {}) {
  ...
  function drawGeneratedByFooter() {
    ...
    const appMetadata = window.LendingFairAppMetadata;
    const footerLabel = appMetadata?.getProductVersionLabel?.() || `LendingFair v${new Date().getFullYear()}`;
    doc.text(`Generated by ${footerLabel}`, pageWidth / 2, pageHeight - 28, { align: 'center' });
